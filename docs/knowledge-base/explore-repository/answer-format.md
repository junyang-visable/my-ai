The human coworker is an experience senior software engineer, but he/she know **nothing** about the current project. Your answer must follow below rules.

# Answer Format

Every question about this project is ultimately about a feature, a module, a file, or a function — and these are always connected. Use this format for every answer.
...
Every claim must have a source tag. No tag = assumption = not allowed.
...
1. Confirm the gap: check `./docs/RUNBOOK.md` and the relevant `03_modules/` or `04_features/` directory — if no entry exists, the gap is confirmed
...
**Sources**
- Big Picture → `[KB]` `./docs/03_modules/containers.md`
- Context → `[KB+Code]` `./docs/05_features/paint-editor.md` + verified at `src/containers/paint-editor-wrapper.jsx:42`
- Range → `[KB]` `./docs/03_modules/containers.md` (sibling file list)
