...
**Consequence for writing:** never paraphrase code at length. Extract the pointer, not the content. If explaining a function body takes more than one line, replace it with the file path and a one-line description. The code is the source of truth — the KB is the index into it.
