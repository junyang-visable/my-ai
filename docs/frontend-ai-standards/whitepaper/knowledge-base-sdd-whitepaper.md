...
解决路径有两条，需同步推进：其一，把项目知识结构化落盘，形成 AI 可检索、可验证的**代码知识库（RepoWiki）**；其二，用**规格驱动开发（SDD）**在动手前先确认规格，使每次实施都站在明文约束上。二者缺一，都会让 AI 在真空中编造。
...
知识库的核心定位是**索引层**，不是叙述层。每一条 RepoWiki 记录做且只做三件事之一：

- 描述全局（一个功能/模块/系统做什么，1–3 行）
- 指向另一个 RepoWiki 文件（跨层级导航链接）
- 指向原始代码（精确文件路径 + 一行角色说明）

这一原则的推论是：如果解释一个函数体超过一行，就用文件路径 + 一行描述替代。代码是真相的来源，RepoWiki 是进入代码的索引。叙述性的 RepoWiki 很快会腐化并误导 AI，而索引性的 RepoWiki 只会「过期」——过期可被检测，误导不易察觉。

### 1.2 可验证性优于完整性

RepoWiki 不追求篇幅或版式，而追求**可验证性**：每一条记录都应能对照代码核实。RepoWiki 引入了 `META.md` 作为版本锚点——记录最后一次同步时的 git commit SHA，使 agent 在每次使用前能精确判断 RepoWiki 是否过期，而不是靠猜测。

### 1.3 写作约束

以下约束对所有 RepoWiki 文件强制执行，无例外：
...
## 二、RepoWiki 目录结构与分层逻辑

RepoWiki 按「知识层次」分层，而非按「话题」分类：
...
## 三、RepoWiki 生命周期：四个 Skill 的协作

RepoWiki 的全生命周期由四个 Agent Skill 覆盖，形成闭环：

```
explore-repository（所有问题的入口）
      │
      ├─ ./repowiki/META.md 不存在 ──► initialize-knowledge-base（从零建库）
      │
      ├─ RepoWiki 过期（last_commit 对不上 HEAD）
      │         └─► refine-knowledge-base（修复 stale 条目）
      │
      └─ RepoWiki 有 gap（问题无法从 repowiki/ 回答）
                └─► refine-knowledge-base（填补缺失）

功能开发完成 ──► archive-knowledge-base（归档新功能）
```

### 3.1 explore-repository：先问 RepoWiki，再看代码
...
Step 0  加载 version-check.md — 用 META.md 中的 last_commit 比对当前 HEAD
        若有 RepoWiki 覆盖的源文件发生变更 → 触发 refine-knowledge-base 后继续
Step 1  打开 RUNBOOK.md — 获取导航起点
Step 2  用 navigation-guide.md 路由到对应 RepoWiki 文件
Step 3  用 answer-format.md 模板组织答案
Step 4  若需读/改代码 → 先跑 verification-protocol.md，对比 RepoWiki 与实际代码
Step 5  若 RepoWiki 有 gap → 触发 refine-knowledge-base，然后继续
Step 6  若读了代码回答了函数问题 → 触发 refine-knowledge-base（gap type=symbol）
```

`explore-repository` 本身**永不直接写 `./repowiki/`**，所有写操作都通过 `refine-knowledge-base` 完成。这个约束确保 RepoWiki 的写入路径是单一的，格式不会因调用方不同而漂移。
...
**触发条件**：`./repowiki/RUNBOOK.md` 不存在（全新项目）。

设计上，主 agent 不直接读源码——所有代码探索由 subagent 完成，主 agent 只接收结构化输出并写 RepoWiki 文件。这个分工防止大量原始代码填满主 agent 的 context。
...
**META.md 在最后写**：确保 `last_commit` 对齐的是 RepoWiki 写完后的代码状态，避免版本锚点与 RepoWiki 内容错位。

### 3.3 archive-knowledge-base：功能交付后归档

**触发条件**：用户发出完成信号（"done"、"PR is up"、"tests pass"）且 `./repowiki/06_features/<feature-name>.md` 尚不存在；或 git push hook 触发 `KB_SYNC_REQUIRED` 信号。
...
Step 4  用 feature-doc-template.md 创建 ./repowiki/06_features/<kebab-name>.md
...
**触发条件**：用户询问的功能/模块/代码路径在 `./repowiki/` 中缺失或不完整。
...
gap 确认后派发一个范围受限的 subagent（只读 Step 2 中确定的最小文件列表），返回结构化 FINDINGS，主 agent 从 FINDINGS 写 RepoWiki——这与 initialize 的主/子分工一致。

`refine-knowledge-base` 是**唯一可以写 `./repowiki/` 的 skill**；`explore-repository` 不写，`archive-knowledge-base` 通过它间接写（实际上 archive 直接写，但遵循同样的写作规范）。
...
需求到达
  │
  ▼
读 RepoWiki（explore-repository）
  → ./repowiki/00_overview/big_picture.md   了解系统全局
  → ./repowiki/01_maps/feature_map.md       确认相关功能边界
  → ./repowiki/04_modules/<name>.md         了解涉及模块的约束
  │
  ▼
产出 Spec（OpenSpec）
  → 功能范围、数据模型、交互逻辑、技术约束
...
archive-knowledge-base
  → 归档 ./repowiki/06_features/<feature>.md
  → 更新 feature_map.md + file_index.md + RUNBOOK.md + META.md
```

**Spec 确认前不动代码**是 SDD 的核心纪律。RepoWiki 读取不是形式——它保证 AI 在制订规格前站在当前项目的真实约束上，而不是依赖当次对话里的零星记忆或自行猜测。
...
代码合并
  + Spec 同步更新
  + RepoWiki 归档（archive-knowledge-base 执行完毕）
...
若 RepoWiki 未归档，下一次 AI 辅助将在过时的上下文中运作，`version-check.md` 会在下次 `explore-repository` 时检测到这一状态并强制触发 `refine-knowledge-base`，但这是被动修复——主动归档的成本远低于事后修复的代价。
...
1. **先建 RepoWiki**（`initialize-knowledge-base`）——有 RepoWiki 才有上下文基础
2. **固化 explore 习惯**（`explore-repository` 作为所有问题的第一步）——建立「先查 RepoWiki」的反射
3. **推广 SDD 节奏**（读 RepoWiki → 产 Spec → 实施 → 归档）——节奏稳定后质量可预期
4. **写入 DoD**（合并后必须跑 archive-knowledge-base）——用流程保底

### 5.2 项目 Owner 职责

- 项目开始时触发 `initialize-knowledge-base`，确认生成的 `META.md` 和 `RUNBOOK.md` 可正常导航
- 评审 `00_overview/big_picture.md` 与 `07_business/`（如有）中的业务知识——这是 RepoWiki 中极少数需要人工补充的部分
...
4. 按计划执行，过程中遇到 RepoWiki gap → `refine-knowledge-base` 自动修复
...
**RepoWiki 不是一次性产物**，它需要与代码同源演进。以下三条节奏有助于保持 RepoWiki 长期有效：

1. **先更新 RepoWiki，再扩大改动范围**：架构或模块边界发生变化时，应先更新 `04_modules/` 与 `01_maps/`，再交给 AI 做大范围编辑。文档滞后时，改动面越大，与真实约定的偏差积累越快。
