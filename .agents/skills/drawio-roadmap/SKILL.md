...
1. **定时间轴**：起止日期 → X0、总天数、右缘 = X0 + 总天数×14；工作日/周末星期行用 `date` 命令校验（如 2026-09-13 → 周日）
   - **未明确指定范围时的默认缓冲**：轴起 = min(今天, 最早任务起始日) − 7 天，轴止 = 最晚任务结束日 + 7 天——前后各多展示一周空白，给后续追加任务/里程碑留展示空间（v1.1 起，2026-09-02 用户拍板）
   - 用户明确给出起止日期时按用户指定，不加缓冲
...
- [ ] 未明确指定时间范围时，时间轴前后各多展示一周（min(今天, 最早任务起始日)−7 ~ 最晚任务结束日+7）
